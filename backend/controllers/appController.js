  ...
  module.exports={	
	...
	search_get: async (req, res) => {
		...
		console.log('query: ',query);
	  	try {
		  const data = await Product.find({ username: { $regex: new RegExp(query, 'i') } }).sort({ createdAt: -1 });
		  res.json(data);
		} catch (err) {
	...
	submit_post:async (req, res)=>{
		try {
			...
			const { base64,category,type,brand,username,price,color,condition,description} = req.body;
			const newProduct = new Product({
				image:base64,category,type,brand,username,price,color,condition,description,user_id
			});
